...
typealias HexColorParsingResult = Pair<Boolean, List<QuantumType>>
...
object HexColor {
    ...
    fun tryParse(value: String): HexColorParsingResult {
        if (value.startsWith('#')) {
    ...
    private fun tryParseQ16(value: String): HexColorParsingResult = try {
        when (value.length) {
            // 12: rrrrggggbbbb
            // 16: rrrrggggbbbbaaaa
            12, 16 -> HexColorParsingResult(true, value.chunked(4) {
