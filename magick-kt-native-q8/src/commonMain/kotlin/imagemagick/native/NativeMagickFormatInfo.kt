...
typealias ExceptionInfoPtrVar = CPointerVar<ExceptionInfo>

@ExperimentalForeignApi
@ExperimentalStdlibApi
object NativeMagickFormatInfo {
    ...
    fun createList(): CreateListResult? = memScoped {
    ...
    fun disposeList(list: CreateListResult) {
        MagickFormatInfo_DisposeList(list.cPointer, list.length.convert())
    }

    @Throws(Exception::class)
    fun getInfo(list: CreateListResult, index: UInt): CPointer<MagickInfo>? = memScoped {
        val exception = alloc<ExceptionInfoPtrVar>()
